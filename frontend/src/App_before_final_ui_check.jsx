...
const Card = ({children,style={}})=>(
  <div className="lewa-card" style={{background:"var(--lewa-surface)",border:"1.5px solid var(--lewa-border)",borderRadius:14,padding:"1.35rem",boxShadow:"0 8px 24px rgba(15,110,86,0.06)",...style}}>{children}</div>
);
const StatCard = ({label,value,sub,color})=>(
  <div className="lewa-stat-card" style={{background:"var(--lewa-surface)",borderRadius:14,padding:"18px 20px",borderLeft:`4px solid ${color||"#0F6E56"}`,border:"1.5px solid var(--lewa-border)",boxShadow:"0 4px 18px rgba(0,0,0,0.04)"}}>
...
function Dashboard({user,areas,shops,profiles,attendance,visits,deliveries,lpos,scores,notifications,authorizations,backup_logs,login_sessions,setPage}){
  ...
  return <div>
    ...
    <Card style={{marginBottom:12}}>
      ...
      {quickAccess.map(item=><button key={item.page} onClick={()=>setPage(item.page)} style={{border:"1.5px solid var(--lewa-border)",background:"var(--lewa-surface)",padding:"13px 14px",borderRadius:12,display:"flex",alignItems:"center",gap:10,justifyContent:"space-between",cursor:"pointer",fontWeight:600,color:"var(--lewa-text)",boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
...
export default function App(){
  ...
  return(
    <div className="lewa-app-shell" style={{display:"flex",height:"100vh",fontFamily:'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif',background:"var(--lewa-shell)",overflow:"hidden"}}>
      ...
      {/* SIDEBAR */}
      <div className={`lewa-sidebar ${sidebarOpen?"is-open":"is-closed"}`} style={{width:sidebarOpen?220:62,flexShrink:0,background:"var(--lewa-surface)",borderRight:"1.5px solid var(--lewa-border)",display:"flex",flexDirection:"column",transition:"width 0.2s ease",overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        <div style={{padding:"12px 14px",borderBottom:"1.5px solid var(--lewa-border)",display:"flex",alignItems:"center",gap:14,minWidth:240}}>
      ...
      <div className="lewa-main-shell" style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",background:"var(--lewa-shell)"}}>
        {/* TOPBAR */}
        <div className="lewa-topbar" style={{height:62,borderBottom:"1.5px solid rgba(255,255,255,0.18)",background:"#0F6E56",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 28px",flexShrink:0,boxShadow:"0 2px 12px rgba(26,107,60,0.18)"}}>
          ...
          <div className="lewa-topbar-actions" style={{display:"flex",alignItems:"center",gap:18,flexWrap:"wrap",justifyContent:"flex-end"}}>
            ...
            <Btn size="sm" variant="ghost" onClick={()=>setThemeMode(m=>m==="dark"?"light":"dark")} style={{minWidth:44,justifyContent:"center"}}>
              <span className="desktop-button-text">{isDark?"Light Mode":"Dark Mode"}</span>
              <span className="mobile-button-icon">{isDark?"☀️":"🌙"}</span>
            </Btn>
        ...
        {mobileMoreOpen&&<div className="lewa-mobile-more-panel" onClick={e=>{if(e.target===e.currentTarget){setMobileMoreOpen(false);setSidebarOpen(false);}}}>
          ...
          <div className="menu-grid">
            {navItems.filter(item=>!["Dashboard","Shops & Locations","Merchandiser Visits","Daily Route Plan"].includes(item)).map(item=>(
              <button key={item} onClick={()=>selectPage(item)} style={{border:"1px solid #E8E8E8",background:"#FFFFFF",borderRadius:14,padding:"14px 12px",display:"flex",alignItems:"center",gap:10,textAlign:"left",boxShadow:"0 6px 18px rgba(0,0,0,0.04)"}}>
