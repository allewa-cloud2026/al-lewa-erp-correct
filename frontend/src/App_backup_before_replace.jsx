...
function VisitsPage({user,shops,profiles,visits,setVisits,areas,canApprove}){
  const [dateF,setDateF]=useState(new Date().toISOString().split("T")[0]);
  const [modal,setModal]=useState(null);
  const [form,setForm]=useState({});

  const filtered=visits.filter(v=>v.visit_date===dateF&&(["Admin","MD","Chief Manager","Monitoring Manager"].includes(user.role)||v.user_id===user.id));

  const checkIn=async shopId=>{
    navigator.geolocation.getCurrentPosition(async pos=>{
      const shop=shops.find(s=>s.id===shopId);
      const dist=shop?.latitude?haversineKm(pos.coords.latitude,pos.coords.longitude,shop.latitude,shop.longitude):0;
      const status=proximityStatus(dist);
      const isFake=dist>0.5;
      const data={user_id:user.id,shop_id:shopId,visit_date:new Date().toISOString().split("T")[0],check_in_time:new Date().toISOString(),check_in_lat:pos.coords.latitude,check_in_lng:pos.coords.longitude,distance_from_shop:Math.round(dist*100)/100,distance_status:status,is_fake_visit:isFake,status:"Checked In",approval_status:"Pending"};
      const rows=await db("merchandiser_visits","insert",{data});
      if(rows.length)setVisits(prev=>[rows[0],...prev]);
      if(isFake)alert(`⚠️ Warning: You are ${dist.toFixed(2)}km from the shop. This visit may be flagged.`);
      else alert("Visit check-in recorded!");
    },()=>alert("GPS required."));
  };
  const approve=async(id,status)=>{
  ...
  return <div>
    <PageHeader title="Shop Visits" subtitle="Check-ins, GPS verification and approval"/>
    <div style={{display:"flex",gap:12,marginBottom:20,alignItems:"center",flexWrap:"wrap"}}>
      <Input type="date" value={dateF} onChange={setDateF} style={{width:180}}/>
      <div style={{display:"flex",gap:12}}>
        <StatCard label="Total" value={filtered.length}/>
        <StatCard label="Confirmed" value={filtered.filter(v=>proximityStatus(v.distance_from_shop)==="Confirmed").length} color="#0F6E56"/>
        <StatCard label="Suspicious" value={filtered.filter(v=>proximityStatus(v.distance_from_shop)==="Suspicious").length} color="#C74C51"/>
        <StatCard label="Pending Approval" value={filtered.filter(v=>v.approval_status==="Pending").length} color="#C4A747"/>
      </div>
    </div>
    <Card style={{padding:0,overflow:"hidden"}}>
      <Table headers={["Staff","Shop","Check-in","Check-out","Distance","Status","Fake?","Approval"]}
        rows={filtered.map(v=>{
          ...
          return [
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Avatar name={p?.full_name} role={p?.role} size={32}/>
              <span style={{fontSize:"13px",fontWeight:"500",color:"#1F1F1F"}}>{p?.full_name}</span>
            </div>,
            <div><div style={{fontSize:"13px",fontWeight:"600",color:"#1F1F1F"}}>{s?.name}</div><div style={{fontSize:"11px",color:"#999999"}}>{s?.branch}</div></div>,
            <span style={{fontSize:"13px"}}>{fmt(v.check_in_time)}</span>,
            <span style={{fontSize:"13px"}}>{fmt(v.check_out_time)||"—"}</span>,
            v.distance_from_shop!=null?<span style={{fontSize:"13px",color:v.distance_from_shop>0.5?"#C74C51":"#666666",fontWeight:v.distance_from_shop>0.5?"600":"400"}}>{v.distance_from_shop}km</span>:<span style={{color:"#999999",fontSize:"13px"}}>—</span>,
            <StatusBadge s={v.status}/>,
            <span style={{fontSize:"13px"}}>{v.is_fake_visit?"⚠️ Yes":"✅ No"}</span>,
            ...
            </div>:<StatusBadge s={v.approval_status}/>
          ];
        })}/>
    </Card>
  </div>;
