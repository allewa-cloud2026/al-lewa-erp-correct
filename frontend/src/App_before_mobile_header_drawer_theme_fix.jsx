...
const NAV_ITEMS = {
  ...
  "MD":["Dashboard","Users & Authorization","Authorizations","Session Tracker","Shops & Locations","Geographic Areas","Product Master","Attendance","Daily Route Plan","Merchandiser Visits","Delivery Tracking","LPO Management","Sales Orders","Daily Scores","Backup Logs","Memo/Documents","Reports","Training / Demo Video","Notifications","Settings"],
  "Chief Manager":["Dashboard","Users & Authorization","Authorizations","Shops & Locations","Geographic Areas","Attendance","Daily Route Plan","Merchandiser Visits","Delivery Tracking","LPO Management","Sales Orders","Daily Scores","Backup Logs","Memo/Documents","Reports","Training / Demo Video","Notifications","Settings"],
  "Monitoring Manager":["Dashboard","Users & Authorization","Authorizations","Shops & Locations","Geographic Areas","Attendance","Daily Route Plan","Merchandiser Visits","Daily Scores","Memo/Documents","Reports","Training / Demo Video","Notifications","Settings"],
  ...
  "Accountant":["Dashboard","Users & Authorization","Shops & Locations","Product Master","Attendance","Delivery Tracking","LPO Management","Sales Orders","Daily Scores","Backup Logs","Memo/Documents","Reports","Training / Demo Video","Notifications","Settings"]
};
...
const Btn = ({children,onClick,variant="default",size="md",style={}})=>{
  ...
  const variants={
    ...
    danger:{background:"#FEF2F2",border:"1.5px solid #E8B8B8",color:"#C74C51","&:hover":{background:"#FEE8E8"}},
    success:{background:"#E8F5F1",border:"1.5px solid #B8E8DA",color:"#0F6E56","&:hover":{background:"#D8F0E7"}},
    gold:{background:"#FEF9E7",border:"1.5px solid #E8D8A8",color:"#A68A2F","&:hover":{background:"#FEF5D8"}},
...
function TrainingVideoPage(){
  return <div>
    ...
    <Card style={{maxWidth:1100,margin:"0 auto",overflowX:"auto"}}>
      <div style={{fontSize:"14px",fontWeight:700,color:"var(--lewa-text)",marginBottom:10}}>Al Lewa Training & Demo Video</div>
      <div style={{border:"2px solid #0B6B2E",borderRadius:16,overflow:"hidden",boxShadow:"0 10px 24px rgba(11,107,46,0.14)",background:"#000000"}}>
        <video
          autoPlay
          muted
          loop
          ...
          controls
          preload="metadata"
          style={{display:"block",width:"100%",maxWidth:"1100px",height:"auto",objectFit:"contain",background:"#000000"}}
          src="/videos/Brand Logo Video.mp4"
        />
      </div>
      <div style={{marginTop:12,fontSize:"12px",color:"var(--lewa-text-muted)"}}>
        This section keeps the video separate so the dashboard stays compact.
      </div>
    </Card>
  </div>;
}
...
function VisitsPage({user,shops,profiles,visits,setVisits,areas,canApprove,daily_routes,daily_route_stops,setDailyRoutes,setDailyRouteStops}){
  ...
  const assignedShops=isManager
    ? shops
    : todayRouteStops.length
      ? todayRouteStops.map(stop=>shops.find(s=>s.id===stop.shop_id)).filter(Boolean)
      : shops.filter(s=>
  ...
  const startVisit=async(routeStop=null)=>{
    ...
    if(todayRouteStops.length&&!selectedStop){
      alert("Start visits from your assigned route list.");
      return;
    }
    ...
    if(todayRouteStops.length&&!routeShopIds.has(shopId)){
      alert("This shop is not assigned to your route today.");
      return;
    }
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10}}>
        <div>
          ...
          <Select value={form.shop_id} onChange={v=>setForm(f=>({...f,shop_id:v,route_stop_id:todayRouteStops.find(s=>s.shop_id===v)?.id||""}))} style={{width:"100%"}}>
            ...
            {assignedShops.sort((a,b)=>(a.route_order||999)-(b.route_order||999)).map(s=><option key={s.id} value={s.id}>{s.route_order?`${s.route_order}. `:""}{s.name} {s.branch?`- ${s.branch}`:""}</option>)}
          </Select>
        </div>
        <div>
...
export default function App(){
  ...
  const [userPasswords,setUserPasswords]=useState({admin:"admin123",ashik:"ashik123",anees:"anees123",shakeel:"shakeel123"});
  ...
  const [daily_routes,setDailyRoutes]=useState(MOCK_DAILY_ROUTES);
  const [daily_route_stops,setDailyRouteStops]=useState(MOCK_DAILY_ROUTE_STOPS);
  ...
  return(
    <div className="lewa-app-shell" style={{display:"flex",height:"100vh",fontFamily:'"Dubai, "Segoe UI", Tahoma, Arial, sans-serif',background:"var(--lewa-shell)",overflow:"hidden"}}>
      ...
      <div className={`lewa-sidebar ${sidebarOpen?"is-open":"is-closed"}`} style={{width:sidebarOpen?236:68,flexShrink:0,background:"linear-gradient(180deg,var(--lewa-surface),var(--lewa-surface-2))",borderRight:"1.5px solid var(--lewa-border)",display:"flex",flexDirection:"column",transition:"width 0.2s ease",overflow:"hidden",boxShadow:"0 8px 22px rgba(0,0,0,0.08)"}}>
        ...
        {sidebarOpen&&DEMO_MODE&&<div style={{padding:"10px 12px",borderBottom:"1.5px solid var(--lewa-border)",background:"var(--lewa-surface-2)"}}>
      ...
      <div className="lewa-main-shell" style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",background:"var(--lewa-shell)"}}>
        {/* TOPBAR */}
        <div className="lewa-topbar" style={{height:58,borderBottom:"1.5px solid rgba(255,255,255,0.16)",background:"linear-gradient(180deg, #0F6E56 0%, #0C5B47 100%)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px",flexShrink:0,boxShadow:"0 6px 18px rgba(8,75,32,0.22)"}}>
          ...
          <div className="lewa-topbar-brand" style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden",flex:1}}>
              <div style={{display:"flex",alignItems:"baseline",gap:10,minWidth:0,flexWrap:"nowrap"}}>
                ...
                <span className="lewa-topbar-page-title" style={{fontSize:"14px",fontWeight:"700",color:"#FFFFFF",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%"}}>{page}</span>
              </div>
              ...
              <span className="lewa-topbar-mobile-title" style={{display:"none",fontSize:"13px",fontWeight:"700",color:"#FFFFFF",marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{`Al Lewa | ${page}`}</span>
            </div>
          </div>
          <div className="lewa-topbar-actions" style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",justifyContent:"flex-end",flexShrink:0}}>
            {DEMO_MODE&&<span className="lewa-demo-badge lewa-topbar-demo-badge" style={{fontSize:"11px",background:"rgba(254,249,231,0.95)",color:"#8D7320",padding:"7px 11px",borderRadius:14,fontWeight:"700",border:"1px solid rgba(232,216,168,0.95)",boxShadow:"0 4px 12px rgba(0,0,0,0.08)"}}>Demo Mode</span>}
      ...
      <div className={`lewa-mobile-more-panel ${mobileMoreOpen?"is-open":""}`} onClick={e=>{if(e.target===e.currentTarget){setMobileMoreOpen(false);setSidebarOpen(false);}}} style={{transform:mobileMoreOpen?"translateX(0)":"translateX(-105%)",pointerEvents:mobileMoreOpen?"auto":"none"}}>
          ...
          <div className="lewa-mobile-drawer-section" style={{display:"none",padding:"10px 0 12px",borderBottom:"1.5px solid var(--lewa-border)"}}>
            <div style={{fontSize:10,color:"var(--lewa-text-muted)",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.08em",fontWeight:700}}>Preview As</div>
            <select value={loginAs} onChange={e=>switchUser(e.target.value)} style={{width:"100%",padding:"10px 12px",border:"1.5px solid var(--lewa-border)",borderRadius:10,background:"var(--lewa-surface-2)",color:"var(--lewa-text)",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              {ROLES.map(r=><option key={r} value={r}>{r}</option>)}
            </select>
          </div>
          ...
          <div className="lewa-mobile-drawer-theme" style={{display:"none",padding:"10px 0 12px",borderBottom:"1.5px solid var(--lewa-border)"}}>
            ...
            <button onClick={()=>setThemeMode(m=>m==="dark"?"light":"dark")} className="theme-toggle" style={{display:"inline-flex",alignItems:"center",gap:10,width:"100%",border:"1.5px solid var(--lewa-border)",borderRadius:14,padding:"10px 12px",background:"var(--lewa-surface-2)",color:"var(--lewa-text)",cursor:"pointer",fontWeight:700}}>
              <span style={{fontSize:17,lineHeight:1}}>{isDark?"🌙":"☀️"}</span>
              <span style={{fontSize:13,whiteSpace:"nowrap"}}>{isDark?"Dark Mode":"Light Mode"}</span>
              <span style={{marginLeft:"auto",display:"inline-flex",alignItems:"center",width:42,height:24,padding:3,borderRadius:999,background:isDark?"#16362D":"rgba(255,255,255,0.55)",border:"1px solid var(--lewa-border)",boxSizing:"border-box"}}>
                <span style={{width:18,height:18,borderRadius:"50%",background:"var(--lewa-surface)",transform:isDark?"translateX(18px)":"translateX(0)",transition:"transform 0.2s ease, background 0.2s ease",boxShadow:"0 2px 6px rgba(0,0,0,0.22)"}} />
              </span>
            </button>
          </div>

          <div className="lewa-mobile-drawer-profile" style={{display:"none",padding:"12px 0 4px"}}>
