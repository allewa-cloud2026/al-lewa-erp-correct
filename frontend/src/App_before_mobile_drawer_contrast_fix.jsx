  ...
  const globalPwaStyles=<style>{`
    ...
    @media (max-width: 767px) {
      ...
      .lewa-mobile-more-panel {
        ...
        padding: 72px 12px 90px !important;
      }
      .lewa-mobile-more-panel .menu-grid {
        display: grid !important;
        grid-template-columns: repeat(2, minmax(0, 1fr)) !important;
        gap: 10px !important;
      }
        ...
        <div className="lewa-topbar" style={{height:58,borderBottom:"1.5px solid rgba(255,255,255,0.16)",background:"linear-gradient(180deg, #0F6E56 0%, #0C5B47 100%)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px",flexShrink:0,boxShadow:"0 6px 18px rgba(8,75,32,0.22)"}}>
          ...
          <div className="lewa-topbar-brand" style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden",flex:1}}>
              ...
              <span className="lewa-topbar-mobile-title" style={{display:"none",fontSize:"13px",fontWeight:"700",color:"#FFFFFF",marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{page}</span>
            </div>
          </div>
      ...
      {typeof window!=="undefined"&&window.innerWidth<768&&<>
        {mobileMoreOpen&&<div className="lewa-mobile-more-panel" onClick={e=>{if(e.target===e.currentTarget){setMobileMoreOpen(false);setSidebarOpen(false);}}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              <div style={{fontSize:18,fontWeight:800,color:"#1F1F1F"}}>More</div>
              <div style={{fontSize:12,color:"#777777",marginTop:2}}>Additional modules</div>
            </div>
            <Btn size="sm" onClick={()=>{setMobileMoreOpen(false);setSidebarOpen(false);}}>Close</Btn>
          </div>
          <div className="menu-grid">
            {navItems.filter(item=>!["Dashboard","Shops & Locations","Merchandiser Visits","Daily Route Plan"].includes(item)).map(item=>(
              <button key={item} onClick={()=>selectPage(item)} style={{border:"1px solid var(--lewa-border)",background:"linear-gradient(180deg,var(--lewa-surface),var(--lewa-surface-2))",borderRadius:14,padding:"14px 12px",display:"flex",alignItems:"center",gap:10,textAlign:"left",boxShadow:"0 8px 18px rgba(0,0,0,0.05)"}}>
                <span style={{fontSize:20,flexShrink:0}}>{MODULE_ICONS[item]||({"Training / Demo Video":"🎬"}[item]||"•")}</span>
                <span style={{fontSize:13,fontWeight:700,color:"#1F1F1F",lineHeight:1.25}}>{item}</span>
              </button>
            ))}
          </div>
        </div>}
