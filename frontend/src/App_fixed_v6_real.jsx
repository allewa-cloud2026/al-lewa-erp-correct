...
const StatCard = ({label,value,sub,color,style={}})=>(
  <div className="lewa-stat-card" style={{background:"#FFFFFF",borderRadius:14,padding:"18px 20px",borderLeft:`4px solid ${color||"#0F6E56"}`,boxShadow:"0 4px 18px rgba(0,0,0,0.04)",...style}}>
...
function VisitsPage({user,shops,profiles,visits,setVisits,areas,canApprove}){
  ...
  const showcaseImages=visits
    .filter(v=>v.before_image_url||v.after_image_url)
    .slice(0,6)
    .flatMap(v=>[
      v.before_image_url?{visit:v,type:"Before",src:v.before_image_url}:null,
      v.after_image_url?{visit:v,type:"After",src:v.after_image_url}:null
    ].filter(Boolean))
    .slice(0,6);
  ...
  const startVisit=async()=>{
    ...
    try{
      ...
      alert(isFake?`Visit started, GPS Suspicious: ${distance} km from shop.`:"Visit started. GPS and check-in time saved.");
    }catch(e){
  ...
  const statStyle={padding:"10px 14px",borderRadius:14,minHeight:72};
  const panelStyle={marginBottom:10,padding:"12px 16px"};

  return <div style={{marginTop:-10}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,marginBottom:10,flexWrap:"wrap"}}>
      <div>
        <h1 style={{fontSize:24,fontWeight:800,color:"#123B2F",margin:"0 0 2px"}}>Field Visit Management</h1>
        <p style={{fontSize:13,color:"#777777",margin:0}}>GPS proof, display photos, comments and approvals</p>
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        <Badge label={isManager?"Management Review":"Field Mode"} color="#0F6E56" bg="#E8F5F1"/>
        {!isManager&&<Badge label={`Assigned shops: ${assignedShops.length}`} color="#0F6E56" bg="#E8F5F1"/>}
      </div>
    </div>

    {showcaseImages.length>0&&<div style={{display:"flex",gap:10,overflowX:"auto",marginBottom:10,padding:"8px 2px"}}>
      {showcaseImages.map((img,i)=>{
        const st=visitStaff(img.visit);
        const sh=visitShop(img.visit);
        return <button key={i} onClick={()=>openPreview(img.visit,img.type)} style={{border:"1px solid #D9EFE7",background:"#FFFFFF",borderRadius:14,padding:0,width:155,height:86,overflow:"hidden",position:"relative",flex:"0 0 auto",boxShadow:"0 4px 14px rgba(15,110,86,0.08)",cursor:"pointer"}}>
          <img src={img.src} alt="Showcase" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          <div style={{position:"absolute",left:0,right:0,bottom:0,background:"linear-gradient(transparent, rgba(0,0,0,.58))",color:"#FFFFFF",fontSize:10,padding:"18px 8px 6px",textAlign:"left"}}>
            <b>{img.type}</b> · {sh?.name||"Shop"}<br/>{st?.full_name?.split(" ")[0]||""}
          </div>
        </button>;
      })}
    </div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,marginBottom:10}}>
      <StatCard label="Visits" value={visibleVisits.length} style={statStyle}/>
      <StatCard label="In Progress" value={visibleVisits.filter(v=>v.status==="in_progress"||v.status==="Checked In").length} color="#C4A747" style={statStyle}/>
      <StatCard label="Completed" value={visibleVisits.filter(v=>v.status==="completed"||v.status==="Completed").length} color="#0F6E56" style={statStyle}/>
      <StatCard label="Suspicious" value={visibleVisits.filter(v=>v.is_fake_visit||visitGpsStatus(v)==="Suspicious").length} color="#C74C51" style={statStyle}/>
      <StatCard label="Pending" value={visibleVisits.filter(v=>v.approval_status==="Pending").length} color="#C4A747" style={statStyle}/>
    </div>

    {isManager&&<Card style={panelStyle}>
      <div style={{fontSize:13,fontWeight:800,color:"#1F1F1F",marginBottom:8}}>Management Filters</div>
    ...
    <Card style={panelStyle}>
      <div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:8}}>
        <div style={{fontSize:15,fontWeight:800,color:"#123B2F"}}>Start Shop Visit</div>
        {activeVisit&&<Badge label="Visit in progress" color="#A68A2F" bg="#FEF9E7"/>}
      </div>

      <div style={{display:"grid",gridTemplateColumns:"minmax(220px,1.4fr) minmax(190px,1fr) minmax(190px,1fr)",gap:10}}>
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:4}}>Shop / Customer</label>
        ...
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:4}}>Before image</label>
          ...
          {form.before_image_url&&<button onClick={()=>setPreviewImage({type:"Selected Before",visit:{...form,shop_id:form.shop_id,user_id:user.id,check_in_time:new Date().toISOString()}})} style={{marginTop:4,border:"none",background:"transparent",color:"#0F6E56",fontWeight:700,cursor:"pointer"}}>📷 View selected</button>}
        </div>
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:4}}>After image</label>
          ...
          {form.after_image_url&&<button onClick={()=>setPreviewImage({type:"Selected After",visit:{...form,shop_id:form.shop_id,user_id:user.id,check_in_time:new Date().toISOString()}})} style={{marginTop:4,border:"none",background:"transparent",color:"#0F6E56",fontWeight:700,cursor:"pointer"}}>📷 View selected</button>}
        </div>
      </div>

      {selectedShop&&<div style={{marginTop:8,background:"#F8FCFA",border:"1px solid #D9EFE7",borderRadius:10,padding:9,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8}}>
    ...
    <div style={{display:"grid",gap:10}}>
      {visibleVisits.length===0&&<Card style={{padding:"18px",textAlign:"center",color:"#888888",fontSize:13}}>No visits for selected filters.</Card>}
      {visibleVisits.map(v=>{
        ...
        return <Card key={v.id} style={{padding:"12px 14px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1.2fr 1.2fr 1fr 0.8fr 1fr 1fr",gap:12,alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Avatar name={p?.full_name} role={p?.role} size={32}/>
              <div><div style={{fontSize:13,fontWeight:800,color:"#1F1F1F"}}>{p?.full_name||"—"}</div><div style={{fontSize:11,color:"#999999"}}>{p?.employee_id||""}</div></div>
            </div>
            <div><div style={{fontSize:13,fontWeight:800,color:"#1F1F1F"}}>{s?.name||v.shop_name||"—"}</div><div style={{fontSize:11,color:"#999999"}}>{s?.branch||""} · {area?.name||"—"}</div></div>
            <div style={{fontSize:12,color:"#555555"}}><b>In:</b> {fmt(v.check_in_time)}<br/><b>Out:</b> {fmt(v.check_out_time)||"—"}<br/><b>Dur:</b> {v.visit_duration_minutes?`${v.visit_duration_minutes} min`:"—"}</div>
            <div><StatusBadge s={gpsStatus}/><div style={{fontSize:11,color:distance>0.5?"#C74C51":"#999999",marginTop:3}}>{distance!=null?`${distance} km`:"—"}</div>{lat&&lng&&<a href={mapLink(lat,lng)} target="_blank" rel="noreferrer" style={{fontSize:12,color:"#0F6E56",fontWeight:700,textDecoration:"none"}}>Map</a>}</div>
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {v.before_image_url?<button onClick={()=>openPreview(v,"Before")} style={{border:"1px solid #B8E8DA",background:"#E8F5F1",color:"#0F6E56",borderRadius:8,padding:"6px 8px",fontWeight:700,cursor:"pointer"}}>Before 📷</button>:<span style={{fontSize:12,color:"#999999"}}>Before —</span>}
              {v.after_image_url?<button onClick={()=>openPreview(v,"After")} style={{border:"1px solid #B8E8DA",background:"#E8F5F1",color:"#0F6E56",borderRadius:8,padding:"6px 8px",fontWeight:700,cursor:"pointer"}}>After 📷</button>:<span style={{fontSize:12,color:"#999999"}}>After —</span>}
            </div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end"}}>
              <Btn size="sm" variant={comment?"success":"gold"} onClick={()=>openPreview(v,"Comment")}>{comment?"💬 View":"💬 Add"}</Btn>
              {canApprove&&v.approval_status==="Pending"?<>
              ...
              </>:<StatusBadge s={v.approval_status||"Pending"}/>}
            </div>
          </div>
        </Card>;
      })}
        ...
        <div style={{display:"grid",gridTemplateColumns:"minmax(260px,1.2fr) minmax(240px,0.8fr)",gap:18}}>
          ...
          <div>
            ...
            <div style={{background:"#FFFFFF",border:"1px solid #E8E8E8",borderRadius:10,padding:10,fontSize:12,color:"#555555",lineHeight:1.6}}>
              ...
              <div><b>Order:</b> {v.order_request||"—"}</div>
            </div>

            <div style={{fontSize:13,fontWeight:700,color:"#1F1F1F",margin:"18px 0 8px"}}>Management Comment</div>
            {isManager?<div>
              <textarea value={commentDraft} onChange={e=>setCommentDraft(e.target.value)} placeholder="Add management comment..." style={{width:"100%",minHeight:90,border:"1px solid #D5D5D5",borderRadius:10,padding:10,fontFamily:"inherit",fontSize:13,boxSizing:"border-box"}}/>
              <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:8}}>
                <Btn size="sm" onClick={()=>setCommentDraft(comment)}>Reset</Btn>
                <Btn size="sm" variant="primary" onClick={saveComment}>Save Comment</Btn>
              </div>
            </div>:<div style={{background:"#F9F9F9",border:"1px solid #E8E8E8",borderRadius:10,padding:10,fontSize:13,color:"#555555"}}>{comment||"No management comment yet."}</div>}
          </div>
        </div>
      </Modal>;
    })()}
...
export default function App(){
  ...
  return(
    <div className="lewa-app-shell" style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif",background:"#F5F5F7",overflow:"hidden"}}>
      ...
      {/* SIDEBAR */}
      <div className={`lewa-sidebar ${sidebarOpen?"is-open":"is-closed"}`} style={{width:sidebarOpen?240:0,flexShrink:0,background:"#FFFFFF",borderRight:"1px solid #E5E5E5",display:"flex",flexDirection:"column",transition:"width 0.2s ease",overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        ...
        <nav style={{flex:1,overflowY:"auto",padding:"10px 8px"}}>
          {navItems.map(item=>(
            <button key={item} onClick={()=>selectPage(item)}
              ...
              {sidebarOpen&&<span style={{flex:1}}>{({"Users & Authorization":"Users","Session Tracker":"Sessions","Shops & Locations":"Shops","Geographic Areas":"Areas","Product Master":"Products","Merchandiser Visits":"Visits","Delivery Tracking":"Delivery","LPO Management":"LPO","Memo/Documents":"Memos","Daily Scores":"Scores"}[item]||item)}{item==="Notifications"&&unread>0&&<span style={{marginLeft:8,background:"#C74C51",color:"#FFFFFF",borderRadius:12,padding:"2px 8px",fontSize:"10px",fontWeight:"600"}}>{unread}</span>}</span>}
            </button>
          ))}
        </nav>
      ...
      <div className="lewa-main-shell" style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",background:"#F5F5F7"}}>
        {/* TOPBAR */}
        <div className="lewa-topbar" style={{height:72,borderBottom:"1px solid rgba(255,255,255,0.18)",background:"linear-gradient(135deg,#0F6E56,#0A4E3F)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 18px",flexShrink:0,boxShadow:"0 2px 10px rgba(15,110,86,0.18)",color:"#FFFFFF"}}>
          <button className="lewa-mobile-menu-btn" onClick={()=>setSidebarOpen(o=>!o)} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,0.14)",border:"1px solid rgba(255,255,255,0.24)",borderRadius:12,fontSize:22,color:"#FFFFFF",cursor:"pointer",padding:"6px 8px"}}>☰</button>
          <div className="lewa-topbar-brand" style={{display:"flex",alignItems:"center",gap:14,flex:1,minWidth:0}}>
            <div className="lewa-topbar-logo" style={{width:48,height:48,display:"flex",alignItems:"center",justifyContent:"center",background:"#FFFFFF",borderRadius:16,overflow:"hidden",border:"1px solid #E5E5E5",flexShrink:0}}>
              <img src={company_settings.company_logo_url||DEFAULT_LOGO} alt="Al Lewa General Trading LLC" style={{width:"100%",height:"100%",objectFit:"contain"}} />
            </div>
            <div style={{display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden"}}>
              <span className="lewa-topbar-company" style={{fontSize:"15px",fontWeight:"800",color:"#FFFFFF",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{company_settings.company_name}</span>
              <span className="lewa-topbar-subtitle" style={{fontSize:"12px",color:"rgba(255,255,255,0.82)",marginTop:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{company_settings.company_subtitle}</span>
            </div>
          </div>
          <div className="lewa-topbar-actions" style={{display:"flex",alignItems:"center",gap:18,flexWrap:"wrap",justifyContent:"flex-end"}}>
            <div className="lewa-topbar-page-title" style={{textAlign:"right"}}>
              <div style={{fontSize:"18px",fontWeight:"800",color:"#FFFFFF"}}>{page==="Merchandiser Visits"?"Visits":page}</div>
              <div style={{fontSize:"12px",color:"rgba(255,255,255,0.78)"}}>Enterprise ERP dashboard</div>
            </div>
            {DEMO_MODE&&<span className="lewa-demo-badge" style={{fontSize:"11px",background:"rgba(255,255,255,0.18)",color:"#FFFFFF",padding:"8px 12px",borderRadius:20,fontWeight:"700",border:"1px solid rgba(255,255,255,0.35)"}}>Demo Mode</span>}
