...
function VisitsPage({user,shops,profiles,visits,setVisits,areas,canApprove}){
  ...
  return <div>
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      <Table headers={["Staff","Shop","Time","GPS","Photos","Actions"]}
        rows={visibleVisits.map(v=>{
          ...
          const photoBtn=(label,available,type)=>available
            ? <button onClick={()=>openPreview(v,type)} style={{border:"1px solid #B8E8DA",background:"#E8F5F1",color:"#0F6E56",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>📷 {label}</button>
            : <span style={{fontSize:12,color:"#999999"}}>{label}: —</span>;
          return [
            <div style={{display:"flex",alignItems:"center",gap:10,minWidth:150}}>
              <Avatar name={p?.full_name} role={p?.role} size={32}/>
              <div><div style={{fontSize:"13px",fontWeight:"700",color:"#1F1F1F"}}>{p?.full_name||"—"}</div><div style={{fontSize:"11px",color:"#999999"}}>{p?.employee_id||""}</div></div>
            </div>,
            <div style={{minWidth:150}}>
              <div style={{fontSize:"13px",fontWeight:"700",color:"#1F1F1F"}}>{s?.name||v.shop_name||"—"}</div>
              <div style={{fontSize:"11px",color:"#999999"}}>{s?.branch||""}{area?.name?` · ${area.name}`:""}</div>
            </div>,
            <div style={{fontSize:"12px",lineHeight:1.6,minWidth:140}}>
              <div><b>In:</b> {fmt(v.check_in_time)}</div>
              <div><b>Out:</b> {fmt(v.check_out_time)||"—"}</div>
              <div style={{color:"#666666"}}><b>Duration:</b> {v.visit_duration_minutes?`${v.visit_duration_minutes} min`:"—"}</div>
            </div>,
            <div style={{fontSize:"12px",lineHeight:1.6,minWidth:120}}>
              <StatusBadge s={gpsStatus}/>
              <div style={{fontSize:11,color:distance>0.5?"#C74C51":"#999999",marginTop:3}}>{distance!=null?`${distance} km`:"—"}</div>
              {lat&&lng?<a href={mapLink(lat,lng)} target="_blank" rel="noreferrer" style={{fontSize:12,color:"#0F6E56",fontWeight:700,textDecoration:"none"}}>📍 Map</a>:null}
            </div>,
            <div style={{display:"flex",gap:8,flexWrap:"wrap",minWidth:150}}>
              {photoBtn("Before",v.before_image_url,"Before")}
              {photoBtn("After",v.after_image_url,"After")}
            </div>,
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",minWidth:170}}>
              {comment
                ? <button onClick={()=>openPreview(v,"Comment")} style={{border:"1px solid #B8E8DA",background:"#E8F5F1",color:"#0F6E56",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:700,cursor:"pointer"}}>💬 View</button>
                : (isManager?<button onClick={()=>openPreview(v,"Comment")} style={{border:"1px solid #E8D8A8",background:"#FEF9E7",color:"#A68A2F",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:700,cursor:"pointer"}}>💬 Add</button>:<span style={{fontSize:12,color:"#999999"}}>No comment</span>)}
