...
// VISITS
function VisitsPage({user,shops,profiles,visits,setVisits,canApprove}){
  const today=new Date().toISOString().split("T")[0];
  ...
  const visibleVisits=visits.filter(v=>{
    const d=v.visit_date || (v.check_in_time?String(v.check_in_time).split("T")[0]:today);
    const owner=v.user_id || v.merchandiser_id;
    return d===dateF && (isManager || owner===user.id);
  });
  ...
  const approve=async(id,status)=>{
    await db("merchandiser_visits","update",{data:{approval_status:status,approved_by:user.id},eq:{id}});
    setVisits(prev=>prev.map(v=>v.id===id?{...v,approval_status:status}:v));
  };
  ...
  return <div>
    <PageHeader title="Merchandiser Visits" subtitle="Shop visit workflow with GPS, photos, notes and manager approval"/>

    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:16,marginBottom:24}}>
      <StatCard label="Today Visits" value={visibleVisits.length}/>
      <StatCard label="In Progress" value={visibleVisits.filter(v=>v.status==="in_progress"||v.status==="Checked In").length} color="#C4A747"/>
      <StatCard label="Completed" value={visibleVisits.filter(v=>v.status==="completed"||v.status==="Completed").length} color="#0F6E56"/>
      <StatCard label="Suspicious GPS" value={visibleVisits.filter(v=>v.is_fake_visit||v.gps_status==="Suspicious"||v.distance_status==="Suspicious").length} color="#C74C51"/>
      <StatCard label="Pending Approval" value={visibleVisits.filter(v=>v.approval_status==="Pending").length} color="#C4A747"/>
    </div>

    <Card style={{marginBottom:24}}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap",alignItems:"center",marginBottom:16}}>
        <div style={{fontSize:16,fontWeight:700,color:"#1F1F1F"}}>🛒 Start Shop Visit</div>
        {!isManager&&<Badge label={`Assigned shops: ${assignedShops.length}`} color="#0F6E56" bg="#E8F5F1"/>}
      </div>

      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:14}}>
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:6}}>Shop / Customer</label>
        ...
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:6}}>Before display image</label>
        ...
        <div>
          <label style={{fontSize:12,fontWeight:700,color:"#666666",display:"block",marginBottom:6}}>After display image</label>
      ...
      {selectedShop&&<div style={{marginTop:12,background:"#F9F9F9",border:"1px solid #E8E8E8",borderRadius:10,padding:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10}}>
        ...
        {selectedShop.latitude&&selectedShop.longitude&&<a href={mapLink(selectedShop.latitude,selectedShop.longitude)} target="_blank" rel="noreferrer" style={{fontSize:12,color:"#0F6E56",fontWeight:700,textDecoration:"none"}}>📍 Open shop map</a>}
      </div>}
      ...
      <div style={{display:"flex",gap:10,marginTop:18,flexWrap:"wrap"}}>
        ...
        {activeVisit&&<span style={{fontSize:13,color:"#0F6E56",fontWeight:700,alignSelf:"center"}}>Visit in progress</span>}
      </div>
    </Card>

    <div style={{display:"flex",gap:12,marginBottom:20,alignItems:"center",flexWrap:"wrap"}}>
      <Input type="date" value={dateF} onChange={setDateF} style={{width:180}}/>
    </div>

    <Card style={{padding:0,overflow:"hidden"}}>
      <Table headers={["Staff","Shop","Check-in","Check-out","Duration","GPS","Map","Status","Before","After","Approval"]}
        rows={visibleVisits.map(v=>{
          const p=profiles.find(x=>x.id===v.user_id||x.id===v.merchandiser_id);
          const s=shops.find(x=>x.id===v.shop_id);
          const distance=v.distance_from_shop;
          const gpsStatus=v.gps_status||v.distance_status||gpsStatusFor(distance);
          const lat=v.latitude||v.check_in_lat;
          const lng=v.longitude||v.check_in_lng;
          return [
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Avatar name={p?.full_name} role={p?.role} size={32}/>
              <span style={{fontSize:"13px",fontWeight:"600",color:"#1F1F1F"}}>{p?.full_name||"—"}</span>
            </div>,
            ...
            v.before_image_url?<span style={{fontSize:"13px",color:"#0F6E56",fontWeight:"700"}}>✅</span>:<span style={{fontSize:"13px",color:"#999999"}}>—</span>,
            v.after_image_url?<span style={{fontSize:"13px",color:"#0F6E56",fontWeight:"700"}}>✅</span>:<span style={{fontSize:"13px",color:"#999999"}}>—</span>,
            canApprove&&v.approval_status==="Pending"?<div style={{display:"flex",gap:6}}>
